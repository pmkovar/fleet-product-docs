...
[IMPORTANT]
====
The `GitRepoRestriction` resource is deprecated and would be removed in a future release. For comprehensive multi-tenant environment governance, it is highly recommended to use the new `Policy` resource instead. For more information, refer to xref:_rstricting_what_tenants_may_do_in_a_namespace[restricting tenant deployments for multi tenancy].
====
